...
public class ExecutorServiceImpl extends BaseServiceImpl implements ExecutorService {
        ...
        switch (executeType) {
            case REPEAT_RUNNING:
                result = insertCommand(loginUser, processInstanceId, processDefinition.getCode(),
                        processDefinition.getVersion(), CommandType.START_PROCESS, startParams);
                break;
    ...
    private int createCommand(CommandType commandType, long processDefineCode, TaskDependType nodeDep,
        ...
        if (commandType == CommandType.COMPLEMENT_DATA) {
            ...
            try {
                ...
                logger.error("create command error:{}",cronParseException);
                return 0;
            }
        } else {
            command.setCommandParam(JSONUtils.toJsonString(cmdParam));
    ...
    protected int createComplementCommandList(String scheduleTimeParam, RunMode runMode, Command command,
        ...
        CommandType commandType=CommandType.COMPLEMENT_DATA_SERIAL_WAIT;
            ...
            if(expectedParallelismNumber!=null)
            {
                Map<String, String> commandParams =
                        StringUtils.isNotBlank(command.getCommandParam()) ? JSONUtils.toMap(command.getCommandParam())
                                : new HashMap<>();
                commandParams.put(CommandKeyConstants.CMD_PARAM_COMPLEMENT_DATA_EXPECTED_PARAL_NUM,String.valueOf(expectedParallelismNumber));
        ...
        if (StringUtils.isNotEmpty(dateList)) {
            ...
            if (!CollectionUtils.isEmpty(listDate)) {
                List<Date> dates= CronUtils.getSelfScheduleDateList(dateList);
                for (Date date : dates) {
